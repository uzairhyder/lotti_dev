...
<title>Lotti Website | @yield('title')</title>
...
<body onload="reset_filters()">
  ...
  @if (session()->has('empty_cart'))
    <script>toastr.error('{{ session()->get('empty_cart') }}')</script>
  @endif
  @if (session()->has('stripe_payment'))
    <script>toastr.success('{{ session()->get('stripe_payment') }}')</script>
  @endif
...
localStorage.removeItem("rating_star");
  ...
  <script>
    ...
    var countries = {{ Illuminate\Support\Js::from(search()) }};
